...
const updateProject = async () => {
  try {
    const { fileUrl } = await upload.uploadFile(projectImg.value)
    oldProject.img = fileUrl
    store.UPDATE_PROJECT(updateId.value, oldProject)
    for (let i in oldProject) oldProject[i] = ''
    updateId.value = null
    toggleModal()
    toast.success('Successfully project added', { autoClose: 1000, theme: 'dark' })
  } catch (e) {
              ...
              <div class="flex items-center justify-center w-full">
                ...
                >
                  <img
                    :src="oldProject.img"
                    alt=""
                    class="absolute left-0 top-0 h-[100px] w-[100px]  object-cover"
                  />
                  <div class="z-10 flex flex-col items-center justify-center pt-5 pb-6">
                    <i class="bx bx-cloud-upload text-5xl"></i>
                    <p class="mb-2 text-sm text-gray-400">
